...
void SoftwareSkinningScene_3::Update()
{
	...
	XMVECTOR transformedBonePosition0{};
	XMVECTOR transformedBonePosition1{};
	...
	for (size_t i = 0; i < m_SkinnedVertices.size(); ++i)
	{
		...
		float currentBlendWeight = m_SkinnedVertices[i].blendWeight0;

		if (currentBlendWeight == 1.f)
		{
			// use the first bone
			transformedVertex = XMVector3TransformCoord(originalVertex, boneTransform0);
		}
		else if (currentBlendWeight == 0.f)
		{
			// use the second bone
		...
		else 
		{
			...
			transformedBonePosition0 = XMVector3TransformCoord(originalVertex, boneTransform0);
			transformedBonePosition1 = XMVector3TransformCoord(originalVertex, boneTransform1);
			transformedVertex = XMVectorLerp(transformedBonePosition0, transformedBonePosition1, currentBlendWeight);
		}
