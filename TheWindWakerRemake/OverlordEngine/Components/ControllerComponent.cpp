...
void ControllerComponent::Initialize(const SceneContext& /*sceneContext*/)
{
	if(!m_IsInitialized)
	{
		// initialize the position and userData
		m_ControllerDesc.position = PxExtendedVec3{ 
			m_pGameObject->GetTransform()->GetPosition().x, 
			m_pGameObject->GetTransform()->GetPosition().y,
			m_pGameObject->GetTransform()->GetPosition().z };
		m_ControllerDesc.userData = m_pController;

		// retrieve the PxControllerManager
		auto controllerManager = GetScene()->GetPhysxProxy()->GetControllerManager();
		// use the manager to create a controller and store it
		m_pController = controllerManager->createController(m_ControllerDesc);
		ASSERT_NULL(m_pController, L"createController called but returned Controller is NULL");

		// Store a pointer of  the ControlerComponent in the userData field of the underlying pxActor
		m_pController->getActor()->userData = this;

		// Set the Collision  group to m_CollisionGroups.word0
		SetCollisionGroup(static_cast<CollisionGroup>(m_CollisionGroups.word0));
		// Set the collision ignore group to m_CollisionGroups.word1
